...
import com.example.testcardproject.bean.EmotionBean;
...
public class SingleEmotionRainView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        matrix.reset();
        float heightScale = mEmotionHeight / bitmap.getHeight();//压缩图标
    ...
    private void initAndResetData(Bitmap bitmap) {
        ...
        bean.bitmap = bitmap;
        bean.x = mRandom.nextInt(1000);
        bean.y = (int)(-Math.ceil(mEmotionHeight));

        float duration = mRandom.nextInt(500)+maxDuration;
        bean.velocityY = (int)(getHeight()*16 / duration);//间隔16毫秒 系统会重绘，这样用*16 比较好
        bean.velocityX = Math.round(mRandom.nextFloat());//nextFloat fanhui 0,1zhijian d zhi
        mEmotionBean = bean;
    }
